...
///Разработка → JPA: Хранение перечислений в базе данных
...
public class BeePerson extends BaseEntity  {
  ...
  //  private PersonGender gender;
   ...
   // private Bee_photo beePhoto;
 /*  @OneToMany(mappedBy="beePerson")
   public List<BeePassport> beePassports = new ArrayList<BeePassport>();

    //from petclinic
    protected  List<BeePassport> getBeePassports() {
        if (this.beePassports == null) {
            this.beePassports = new ArrayList<BeePassport>();
        }
        return this.beePassports;
    }

    public void setBeePassport(BeePassport beePassport) {
        getBeePassports().add(beePassport);
        beePassport.setBeePerson(this);
    }

    public int getNrOfBeePassports() {
        return getBeePassports().size();
    }*/
